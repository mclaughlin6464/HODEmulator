...
#Name idea: Zoo, since it holds all the animals. Idk.
...
# different catalog formats have different structures
# these define that for Halotools
...
class Emu(Cat):
    ...
    def __init__(self, Lbox, **kwargs):
        ...
        # different defaults depending on the size.
        if Lbox == 1050.0:
            pass
        elif Lbox == 200.0:
            ...
            tmp_fnames = glob(kwargs['loc'] + 'hlist_*.list')  # snag all the hlists
            default200['filenames'] = [fname[len(kwargs['loc']):] for fname in
                                       tmp_fnames]  # just want the names in the dir
        ...
        tmp_scale_factors = defaults['scale_factors']
        tmp_fnames = defaults['filenames']
        # if the user passed in certain scale factors or filenames, take just those.
        self.update_lists(kwargs, tmp_fnames, tmp_scale_factors)
...
class Fox(Cat):
    def __init__(self, Lbox, **kwargs):
        ...
        #only one box size available for now
        if Lbox == 400.0:
            pass
        else:
        ...
        tmp_scale_factors = defaults['scale_factors']
        tmp_fnames = defaults['filenames']
        #if the use specified specific filenames or scale factors, select only those.
        self.update_lists(kwargs, tmp_fnames, tmp_scale_factors)
...
class Guppy(Cat):
    def __init__(self, Lbox, **kwargs):
        ...
        # Only one boxsize is available for Guppy
        if Lbox == 1050.0:
            pass
        else:
        ...
        tmp_scale_factors = defaults['scale_factors']
        tmp_fnames = defaults['filenames']
        #if the use specified some filenames or scale factors, select only those.
        self.update_lists(kwargs, tmp_fnames, tmp_scale_factors)
...
class MDHR(Cat):#TODO put this under Multidark? Ugh....
    def __init__(self, Lbox, **kwargs):
